...
namespace Linq.EnumerableMethods
{
    public class SumMaxMinMethod
    {
        ...
        {
            var englishAlbumList = (from album in bandsList
                                    where album.Country == "England"
                                    select album.Albums.Count()).Sum();

            return bandsList.Where(item => item.Country == "England").Select(item => item.Albums.Count()).Sum();
          //  return (int)englishAlbumList;
        }
    }
}