...
namespace Linq
{
    class Program
    {
        ...
        {
            var bandsthatcontainEnglandCountry = ContainsMethod.GetBandsThatContainsEnglandCountryClassic(bandsList);

            foreach (var item in bandsthatcontainEnglandCountry)
            {
                Console.WriteLine("{0},{1}" ,item.Country, item.Name);
            }

            var bandsThatContainEnglandCountryLambda = ContainsMethod.GetBandsThatContainsEnglandCountryLambda(bandsList);
            Console.WriteLine();
            foreach (var item in bandsThatContainEnglandCountryLambda)
            {
                Console.WriteLine("{0},{1}", item.Name, item.Country);
            }

            var bandsThatContainEnglandCountryLinq = ContainsMethod.GetBandsThatContainsEnglandCountryLinq(bandsList);

            Console.WriteLine();
            foreach(var item in bandsThatContainEnglandCountryLinq)
            {
                Console.WriteLine("{0} , {1}", item.Name, item.Country);
            }
        }
        private static void OrderByMethodExample()
        {
            Console.WriteLine("=====Homework 1 (Order by method)=====");
            Console.WriteLine();

            var orderByCountryClassic = OrderByMethod.GetBandsSortedByCountryClassic(bandsList);
            
            foreach(var item in orderByCountryClassic)
            {
                Console.WriteLine("{0},{1}", item.Name, item.Country);
            }
            Console.WriteLine();
            var orderByCountryLambda = OrderByMethod.GetBandsSortedByCountryLambda(bandsList);
         
            foreach(var item in orderByCountryLambda)
            {
                Console.WriteLine("{0},{1}", item.Name, item.Country);
            }

            var orderbyCountryLinq = OrderByMethod.GetBandsSortedByCountryLinq(bandsList);

            foreach(var item in orderbyCountryLinq)
            {
                Console.WriteLine("{0},{1}", item.Name, item.Country);
            }

            var orderbyNumberOfAlbums = OrderByMethod.GetBandsSortedByNumberOfAlbums(bandsList);
            Console.WriteLine();
            foreach (var item in orderbyNumberOfAlbums)
            {
                Console.WriteLine("{0},{1},{2}", item.Name, item.Country, item.Albums.Count());
            }
            Console.WriteLine();
            Console.Read();
        }
        ...
        private static void AggregateExample()
        {
            ...
            Console.Write("Sum of even: {0}",sumOfEven);
